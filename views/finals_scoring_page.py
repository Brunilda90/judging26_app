...
def _render_scoring_form(judge_id, comp_id: str, comp_name: str, questions, view_only: bool):
    ...
    highlight_key     = f"finals_highlight_{judge_id}_{comp_id}"
    ...
    with form_ctx:
        ...
        for i, q in enumerate(questions, 1):
            ...
            # Question card header — red left-border highlight + scroll anchor when missing
...
def show():
    ...
    # ── Persist selected finalist across reruns ──────────────────────────────
    _sel_key  = f"finals_team_sel_{judge_id}"
    _name_key = f"finals_sel_name_{judge_id}"
    ...
    selected_label = st.selectbox(
        "Select a finalist team to score", option_labels,
        label_visibility="collapsed", key=_sel_key,
    )
    st.session_state[_name_key] = comp_by_label[selected_label]["competitor_name"]
